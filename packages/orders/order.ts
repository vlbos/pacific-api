...
import ordersJSONFixture from './orders.json';
 ...
 */
export var flattenObjectx = function (ob: any): any {
    ...
    for (var i in ob) {
        ...
        if ((typeof ob[i]) === 'object') {
            ...
            for (var x in flatObject) {
                ...
                toReturn[i + (!!isNaN(Number(x)) ? '.' + x : '')] = flatObject[x];
            }
        } else {
            toReturn[i] = ob[i];
        }
...
export const arr2obj = (arr: any) => {
    let obj = {};
    arr.reduce((o: any, a: any) => { o[a["name"]] = a["value"]; return o; }, obj);

    return obj;
}


export const orderFieldsJSONToOrder = (orderFieldsJSON: any) => {
    let obj = {};
    console.log("orderFieldsJSON====",orderFieldsJSON)
    if (null == orderFieldsJSON||null == orderFieldsJSON.fields) {
        return orderFieldsJSON
    }
    orderFieldsJSON.fields.reduce((o: any, a: any) => { o[a["name"]] = a["value"]; return o; }, obj);
    Object.assign(orderFieldsJSON, unflattenObject(obj))
    delete orderFieldsJSON.fields
...
const orderJSONToHexArray = (orderJSON: any) => {
    let obj: { [index: string]: any } = flattenObject(orderJSON);
    let arr: any = [];
    Object.keys(obj).reduce((a, o) => {
        a.push([stringToHex(o), stringToHex(obj[o] + "")]);
        return a;
    }, arr);
...
export { orderJSONToHexArray, makeOrderArrayEx, makeOrderArrayHexEx, makeOrderFromJSONHex, makeOrderEx, makeOrder, orderFromJSON };