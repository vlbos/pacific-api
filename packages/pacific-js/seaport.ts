...
// const ERC20 = require("./abisv2/erc20/metadata.json");
...
import * as WyvernSchemas from '../wyvern-schemas/index'
...
import {
    ...
    estimateCurrentPrice, getOrderHash,
    delay, assignOrdersToSides,
...
import {
    ...
    // DEFAULT_WRAPPED_NFT_LIQUIDATION_UNISWAP_SLIPPAGE_IN_BASIS_POINTS,MAINNET_PROVIDER_URL,
    INVERSE_BASIS_POINT,
...
export class OpenSeaPort {
    ...
    public apiPro: any = "";//new ApiPromise({})
    public apiProReadOnly: any = "";// new ApiPromise({})
    public provider: WsProvider
    public readonlyProvider: WsProvider
     ...
     *  information
     */
    constructor(provider: WsProvider, api: ApiPromise, apiConfig: OpenSeaAPIConfig = {}, logger?: (arg: string) => void) {
        ...
        // // WyvernJS config
        // this._wyvernProtocol = this.apip.tx;
    ...
    //     const wrappedNFTFactoryContract = this.apip.eth.contract(WrappedNFTFactory as any[])
    ...
    //     const wrappedNFTContract = this.apip.eth.contract(WrappedNFT as any[])
    //     const wrappedNFT = await wrappedNFTContract.at(wrappedNFTAddress)
    //     const uniswapFactoryContract = this.apip.eth.contract(UniswapFactory as any[])
    ...
    //     const uniswapExchangeContract = this.apip.eth.contract(UniswapExchange as any[])
    ...
    public async cancelOrder(
        ...
        // const gasPrice = await this._computeGasPrice()
        const transactionHash = this._wyvernProtocol.wyvernExchange.cancelOrderEx(
            ...
            order.v).signAndSend(accountPair)
    ...
    public async approveSemiOrNonFungibleToken(
        ...
        const erc20abi = new Abi(tokenAbi, this.apiPro.registry.getChainProperties());
    ...
    public async isAssetTransferrable(
        ...
        try {
            let gas;
            {
                // Perform the actual read (no params at the end, for the `get` message)
                // (We perform the send from an account, here using Alice's address)
                const erc20abi = new Abi(, this.apiPro.registry.getChainProperties());
                const contract = new ContractPromise(this.apiPro, erc20abi, abi.target);
                // const address = "5GeW32zNDAPvUzRPKhpNjHR2e6ZvcsHdvFzJy6XcffQEbJbu";
    ...
    public async getAssetBalance(
        ...
        if (schema.functions.countOf) {
            ...
            let owner;
            {
                // Perform the actual read (no params at the end, for the `get` message)
                // (We perform the send from an account, here using Alice's address)
                const erc721abi = new Abi(ERC721, this.apiPro.registry.getChainProperties());
                const contract = new ContractPromise(this.apiPro, erc721abi, abi.target);
    ...
    public async computeFees(
        ...
        if (side == OrderSide.Sell && asset) {
            ...
            try {
                ...
                // const result = await getTransferFeeSettings(this.apip, { asset, accountAddress })
                transferFee = new BigNumber(0)// result.transferFee != null ? result.transferFee : transferFee
    ...
    public async validateAndPostOrder(order: Order): Promise<Order> {
        const hash = await this._wyvernProtocolReadOnly.wyvernExchange.hashOrderEx(
            [order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken],
            [order.makerRelayerFee.toNumber(), order.takerRelayerFee.toNumber(), order.makerProtocolFee.toNumber(), order.takerProtocolFee.toNumber(), order.basePrice.toNumber() / Number(1000000000), order.extra.toNumber(), order.listingTime.toNumber(), order.expirationTime.toNumber(), order.salt.toNumber()],
            order.feeMethod,
            order.side,
    ...
    public async _computeGasPrice(): Promise<BigNumber> {
        // const meanGas = await getCurrentGasPrice(this.apip)
        // const weiToAdd = this.apip.toWei(this.gasPriceAddition, 'gwei')
    ...
    public async _estimateGasForMatch(
        ...
        const sell_hash = await this._wyvernProtocolReadOnly.wyvernExchange.hashToSignEx(
            [sell.exchange, sell.maker, sell.taker, sell.feeRecipient, sell.target, sell.staticTarget, sell.paymentToken],
            [sell.makerRelayerFee.toNumber(), sell.takerRelayerFee.toNumber(), sell.makerProtocolFee.toNumber(), sell.takerProtocolFee.toNumber(), sell.basePrice.toNumber() / Number(1000000000), sell.extra.toNumber(), sell.listingTime.toNumber(), sell.expirationTime.toNumber(), sell.salt.toNumber()],
            sell.feeMethod,
            sell.side,
            sell.saleKind,
            sell.howToCall,
        ...
        const buy_hash = await this._wyvernProtocolReadOnly.wyvernExchange.hashToSignEx(
            [buy.exchange, buy.maker, buy.taker, buy.feeRecipient, buy.target, buy.staticTarget, buy.paymentToken],
            [buy.makerRelayerFee.toNumber(), buy.takerRelayerFee.toNumber(), buy.makerProtocolFee.toNumber(), buy.takerProtocolFee.toNumber(), buy.basePrice.toNumber() / Number(1000000000), buy.extra.toNumber(), buy.listingTime.toNumber(), buy.expirationTime.toNumber(), buy.salt.toNumber()],
            buy.feeMethod,
            buy.side,
            buy.saleKind,
            buy.howToCall,
        ...
        try {
            const info = await this._getClientsForRead(retries).wyvernProtocol.wyvernExchange.atomicMatchEx(args[0],
                args[1], args[2], args[3], args[4], args[5],
                args[6], args[7], args[8], args[9], args[10], args[11]).paymentInfo(accountPair)
    ...
    public async _makeBuyOrder(
        ...
        return {
            ...
            metadata: {
                asset: wyAsset,
                schema: "ERC20" as WyvernSchemaName,
                referrerAddress
            }
        }
    }
    ...
    public async _getStaticCallTargetAndExtraData(
        ...
        //     const decentralandEstateABI = this.apip.eth.contract(DecentralandEstates as any[])
        //     const decentralandEstateInstance = await decentralandEstateABI.at(decentralandEstateAddress)
        //     const estateFingerprint = await rawCall(this.apip, {
    ...
    public async _makeBundleBuyOrder(
        { assets, collection, quantities, accountAddress, startAmount, expirationTime = 0,
            paymentTokenAddress, extraBountyBasisPoints = 0, sellOrder, referrerAddress }:
            {
                assets: Asset[];
                collection?: { slug: string };
                quantities: number[];
        ...
        const { calldata, replacementPattern } = { "calldata": "", "replacementPattern": "" }
        // encodeAtomicizedBuy(orderedSchemas, bundle.assets,
        //     accountAddress, this._wyvernProtocol, this._networkName)

        const { basePrice, extra, paymentToken } =
            await this._getPriceParameters(OrderSide.Buy,
                paymentTokenAddress, expirationTime, startAmount)
        const times = this._getTimeParameters(expirationTime)
    ...
    public async _makeBundleSellOrder(
        ...
        const { calldata, replacementPattern } = { "calldata": "", "replacementPattern": "" }// "encodeAtomicizedSell(orderedSchemas, bundle.assets, accountAddress, this._wyvernProtocol, this._networkName)"
    ...
    public async _approveOrder(order: UnsignedOrder) {
        ...
        const transactionHash = this._wyvernProtocol.wyvernExchange.approveOrderEx(
            [order.exchange, order.maker, order.taker, order.feeRecipient, order.target, order.staticTarget, order.paymentToken],
            [order.makerRelayerFee, order.takerRelayerFee, order.makerProtocolFee, order.takerProtocolFee, order.basePrice, order.extra, order.listingTime, order.expirationTime, order.salt],
            order.feeMethod,
            order.side,
    ...
    private async _getPriceParameters(
        ...
        const isToken = tokenAddress == NULL_ADDRESS
        ...
        if (isToken && orderSide === OrderSide.Buy) {
    ...
    private async _authorizeOrder(
        ...
        const makerIsSmartContract = await isContractAddress(this.apip, signerAddress)
        ...
        try {
            if (makerIsSmartContract) {
                ...
                return null // await personalSignAsync(this.apip, message, signerAddress)
            }
        } catch (error) {
    ...
    private async _confirmTransaction(transactionHash: string, event: EventType, description: string, testForSuccess?: () => Promise<boolean>): Promise<void> {
        ...
        try {
            ...
            await confirmTransaction(this.apip, transactionHash)
