...
export const encodeSell: Encoder = (schema, asset, address, wyvernProtocol, networkName) => {
    ...
    const {atomicizedCalldata, atomicizedReplacementPattern }:any = encodeAtomicizedCalldata(atomicizer, [schema], [asset], address, OrderSide.Sell);//encodeCall(atomicizer, ["0b396f18",transfer.target,address, DummyNullAddress, asset.id == undefined ? (<WyvernFTAsset>asset).quantity : asset.id])
    return {
...
export const encodeAtomicizedSell: AtomicizedSellEncoder = (schemas, assets, address, wyvernProtocol, networkName) => {
    ...
    const { atomicizedCalldata, atomicizedReplacementPattern }:any = encodeAtomicizedCalldata(atomicizer, schemas, assets, address, OrderSide.Sell)
...
export const encodeAtomicizedBuy: AtomicizedBuyEncoder = (schemas, assets, address, wyvernProtocol, networkName) => {
    ...
    const { atomicizedCalldata, atomicizedReplacementPattern } :any= encodeAtomicizedCalldata(atomicizer, schemas, assets, address, OrderSide.Buy)
...
export const encodeBuy: Encoder = (schema, asset, address, wyvernProtocol, networkName) => {
    ...
    const  {atomicizedCalldata, atomicizedReplacementPattern }:any = encodeAtomicizedCalldata(atomicizer, [schema], [asset], address, OrderSide.Buy);//encodeCall(token, ["0b396f18",transfer.target,DummyNullAddress, address, asset.id == undefined ? (<WyvernFTAsset>asset).quantity : asset.id])
