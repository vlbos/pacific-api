...
class WyvernProtocol {
    constructor(provider, api, config) {
        ...
        //     {},
        // )
        const regabi = new api_contract_1.Abi(wyvern_proxy_registry_metadata, this.api.registry != undefined ? this.api.registry.getChainProperties() : undefined);
        ...
        const mabi = new api_contract_1.Abi(msigmetadata, this.api.registry != undefined ? this.api.registry.getChainProperties() : undefined);
    ...
    static getOwnableDelegateProxyAddress(network) {
    ...
    static isValidSignature(data, signature, signerAddress) {
        ...
        const isValidSignature = true; //signatureUtils.isValidSignature(data, signature, signerAddress)
    ...
    static generatePseudoRandomSalt() {
        ...
        const salt = new bignumber_js_1.default(10); // randomNumber.times(factor).round()
        return salt;
    }
    /**
    ...
    static isValidOrderHash(orderHash) {
        ...
        const isValidOrderHash = true; //schemaValidator.validate(orderHash, schemas.orderHashSchema).valid
    ...
    static toUnitAmount(amount, decimals) {
        ...
        const aUnit = new bignumber_js_1.default(10).pow(decimals);
    ...
    static toBaseUnitAmount(amount, decimals) {
        ...
        // assert.isNumber('decimals', decimals)
        const unit = new bignumber_js_1.default(10).pow(decimals);
    ...
    static getOrderHashHex(order) {
        ...
        const orderHashHex = utils_1.utils.getOrderHashHex(order);
        return orderHashHex;
    }
    /**
    ...
    static getAssetHashHex(assetHash, schema) {
        const assetHashHex = utils_1.utils.getAssetHashHex(assetHash, schema);
        return assetHashHex;
    }
    getProvider() {
