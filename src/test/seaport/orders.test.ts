...
import { Network, OrderJSON, OrderSide, Order, SaleKind, UnhashedOrder, UnsignedOrder, Asset, WyvernSchemaName } from '../../pacific-js/types'
...
describe('seaport: orders', () => {
    ...
    let s = Object.values(ordersJSON) as OrderJSON[]
    s.map((orderJSON: OrderJSON, index: number) => {
    ...
    it('Matches a referred order via sell_orders and getAssets', async () => {
        ...
        const asset = assets.filter((a:any) => !!a.sellOrders)[0]
