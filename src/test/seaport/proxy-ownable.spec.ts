...
describe("proxy Smart Contracts registry", () => {
    ...
    test("initialize", async () => {
        const registryAddress:any = addresses["registry"]
    ...
    test.only("proxy", async () => {
        ...
        let calldata:any = erc20.tx.transferFrom({ value: new BN(0), gasLimit: new BN(-1) }, from, to, value).toHex();
...

        {
            ...
            // gas consumed
            console.log(gasConsumed.toString());
            gasLimit =new BN(gasConsumed.toString()).mul (new BN(100));// new BN(99722) * (new BN(1_000_000));//
            // check if the call was successful
            if (result.isOk) {
