...
// import { orderJSONToHexArray} from '../../orders/order'
...
export const orderQueryToJSON = (order: any): any => {
    ...
    let asJSON: { [index: string]: any } = {}

    const orderQueryMinProps = ["owner",
        "token_ids",
        "limit",
        "offset"]
    orderQueryMinProps.map((o: any) => {
...
//:{asset: WyvernAsset, i:Number}
      const wyAssets = assets.map((asset:any, i:any) => getWyvernAsset(schemas[i], asset, quantities[i]))
      ...
      const wyAssetsAndSchemas = wyAssets.map((asset:any, i:any) => ({
        asset,
        schema: schemas[i].name as WyvernSchemaName
      }))

      const uniqueAssets = _.uniqBy(wyAssetsAndSchemas, (group:any) => `${sorters[0](group)}-${sorters[1](group)}`)
